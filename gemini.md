...
### 8. Bonus System, Enemy Flight Objects & Homing Missiles
...
- **Rescue Streak:** Consecutive astronaut rescues inside a short time window build a separate rescue multiplier.
- **SkyRaider Flight Objects:** Top-entry hostile craft spawn as `stalker` (tracking/aimed pressure) or `lancer` (faster strike pressure with higher reward).
- **Elite Drone:** Rare non-standard target; rescue or destroy it to gain a permanent run perk.
- **Dynamic Mini-Events:** Timed battlefield modifiers (`SOLAR STORM`, `LOW GRAVITY`, `SWARM RUSH`) periodically alter flow and pressure.
...
- **Content:** Controls, power-ups, heat management, scoring, world events, and enemy flight object behavior are summarized in a scrollable, masked panel (touch-draggable).
